...
public class zFoxVirtualPad : MonoBehaviour
{
	...
	public Vector2 autoLayoutPOS_ButtonA = new Vector2(0.8f, 0.5f);
	public Vector2 autoLayoutPOS_ButtonB = new Vector2(0.65f, 0.35f);
	public Vector2 autoLayoutPOS_ButtonX = new Vector2(0.65f, 0.65f);
	public Vector2 autoLayoutPOS_ButtonY = new Vector2(0.5f, 0.5f);
	...
	public zFOXVPAD_BUTTON buttonX = zFOXVPAD_BUTTON.NON;
	public zFOXVPAD_BUTTON buttonY = zFOXVPAD_BUTTON.NON;
	...
	SpriteRenderer sprButtonX;
	SpriteRenderer sprButtonY;
	...
	bool buttonXHit = false;
	bool buttonYHit = false;
	...
	void Awake()
	{
		...
		sprButtonX = GameObject.Find("Button_X").GetComponent<SpriteRenderer>() as SpriteRenderer;
		sprButtonY = GameObject.Find("Button_Y").GetComponent<SpriteRenderer>() as SpriteRenderer;
	...
	void RunAutoLayout()
	{
		...
		if (autoLayout)
		{
			...
			Vector3 posBtnX = new Vector3(scPos.x * autoLayoutPOS_ButtonX.x, -scPos.y * autoLayoutPOS_ButtonX.y, 0.0f);
			sprButtonX.transform.localPosition = posBtnX;
			Vector3 posBtnY = new Vector3(scPos.x * autoLayoutPOS_ButtonY.x, -scPos.y * autoLayoutPOS_ButtonY.y, 0.0f);
			sprButtonY.transform.localPosition = posBtnY;
		}
	}
	...
	void Update()
	{
        ...
        if (GameManager.instance.isControl == false)
        {
			buttonA = zFOXVPAD_BUTTON.NON;
			 ...
			 buttonX = zFOXVPAD_BUTTON.NON;
			 buttonY = zFOXVPAD_BUTTON.NON;
		}
		...
		if (buttonX == zFOXVPAD_BUTTON.UP)
		{
			buttonX = zFOXVPAD_BUTTON.NON;
			buttonAindex = -1;
		}
		if (buttonY == zFOXVPAD_BUTTON.UP)
		{
			buttonY = zFOXVPAD_BUTTON.NON;
			buttonBindex = -1;
		}
		...
		if (Input.touchCount > 0)
		{
			...
			for (int i = 0; i < Input.touchCount; i++)
			{
				if (i != buttonAindex && i != buttonBindex && i != buttonXindex && i != buttonYindex)
				{
					TouchPhase tp = Input.GetTouch(i).phase;
					if (tp == TouchPhase.Began)
					{
	...
	void CheckButtonDown(RaycastHit hit, int i)
	{
		...
		if (hit.collider.gameObject == sprButtonX.gameObject)
		{
			buttonX = zFOXVPAD_BUTTON.DOWN;
			buttonXindex = i;
			buttonXHit = true;
			sprButtonX.color = new Color(1.0f, 0.0f, 0.0f);
		}
		else
		if (hit.collider.gameObject == sprButtonY.gameObject)
		{
			buttonY = zFOXVPAD_BUTTON.DOWN;
			buttonYindex = i;
			buttonYHit = true;
			sprButtonY.color = new Color(1.0f, 0.0f, 0.0f);
		}
	}
	...
	void CheckButtonUp(RaycastHit hit, int i)
	{
		...
		if (hit.collider.gameObject == sprButtonX.gameObject)
		{
			buttonX = zFOXVPAD_BUTTON.UP;
			buttonXindex = i;
			sprButtonX.color = new Color(1.0f, 1.0f, 1.0f);
		}
		else
		if (hit.collider.gameObject == sprButtonY.gameObject)
		{
			buttonY = zFOXVPAD_BUTTON.UP;
			buttonYindex = i;
			sprButtonY.color = new Color(1.0f, 1.0f, 1.0f);
		}
	}
